...
class TownSquare:
    ...
    def assign(self, character):
        ...
        if not any(
            not_in_play_character.name == character.name
            for not_in_play_character in self.not_in_play_characters
        ):
            return False, WarningAddCharacterNotNotInPlay.format(character.name)

        # Verify the character isn't somehow already in play
        if any(
            in_play_character.name == character.name
            for in_play_character in self.in_play_characters
        ):
        ...
        self.not_in_play_characters = list(
            filter(lambda x: x.name == character.name, self.not_in_play_characters)
        )
